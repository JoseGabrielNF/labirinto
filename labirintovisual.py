...
# variaveis globais
metodo=" busca\n em\n largura   "         # variavel que salva o metodo sendo utilizado
heuristica="distancia\neuclidiana"        # variavel que salva a heuristica sendo utilizada
...
def h2(inicio,fim):
    x1,y1=inicio                            # extraindo posição inicial
    x2,y2=fim                               # extraindo posição final
    distancia = abs(x1-x2) + abs(y1-y2)     # calculando distancia Manhattan 
    return distancia
...
def alterarHeuristica(botao):
    global heuristica
    if(botao['text']=="distancia\neuclidiana"):
        heuristica="distancia\nmanhattan"
    elif(botao['text']=="distancia\nmanhattan"):
        heuristica="distancia\neuclidiana"

    botao['text']=heuristica
...
def encontrarCaminho():
    ...
    funcaoheuristica=None

    if heuristica=='distancia\neuclidiana':
        funcaoheuristica=h1
    elif heuristica=='distancia\nmanhattan':
        funcaoheuristica=h2

    if metodo == " busca\n em\n largura   ":
        caminho=busca_em_largura(inicio,fim,matriz)
    elif metodo == "busca\nheuristica":
        caminho=busca_heuristica(inicio,fim,matriz,funcaoheuristica)
    elif metodo == "a estrela  ":
        caminho=a_estrela(inicio,fim,matriz,funcaoheuristica)
...
def slide(var):
    ...
    quadrados=quadrados[(11):]                  # pegando tudo que não faz parte da interface, que são os quadrados
...
Inicio.place(x=xpos,y=ypos,height=50)
xpos+=40
Fim=Button(root,text="Fim",bg='green',command=corVerde)
Fim.place(x=xpos,y=ypos,height=50)
xpos+=30
Obstaculo=Button(root,text="Obstaculo",bg='blue',command=corAzul)
Obstaculo.place(x=xpos,y=ypos,height=50)
xpos+=60
Branco=Button(root,text="Branco",bg='white',command=corBranca)
Branco.place(x=xpos,y=ypos,height=50)
xpos+=48
escolherMetodo=Button(root,text=" busca\n em\n largura   ",bg='gray')
escolherMetodo.config(command=lambda button=escolherMetodo:alterarMetodo(button)) 
escolherMetodo.place(x=xpos,y=ypos,height=50)
xpos+=58
escolherHeuristica=Button(root,text="distancia\neuclidiana",bg='gray')
escolherHeuristica.place(x=xpos,y=ypos,height=50)
escolherHeuristica.config(command=lambda button=escolherHeuristica:alterarHeuristica(button)) 
xpos+=63
encontrarCaminho=Button(root,text="Encontrar\nCaminho",bg='orange',command=encontrarCaminho)
encontrarCaminho.place(x=xpos,y=ypos,height=50)
xpos+=66
velocidade = Scale(root,from_=0,to=500,orient= HORIZONTAL,command=alterartempo,length=300)
velocidade.place(x=xpos,y=ypos,height=50)
xpos+=100
labelVelocidade=Label(root,text="Lentidão")
labelVelocidade.place(x=xpos,y=ypos-15)
